...
geojson_polygon = r'C:\Users\ecarle\OneDrive - MetService\Documents\2_Consutancy\projects\tauranga\CoastSat\data\MATAKANA_All_1984_2024\gis\Matakana_Island.geojson'
...
# date range
dates = dates = ['1984-03-01', '2024-03-24']

# satellite missions
sat_list = ['L5','L7','L8', 'L9', 'S2']
collection = 'C02' # choose Landsat collection 'C01' or 'C02'
# name of the site
sitename = 'MATAKANA_All_1984_2024'

# filepath where data will be stored
filepath_data = r"C:\Users\ecarle\OneDrive - MetService\Documents\2_Consutancy\projects\tauranga\CoastSat\data"
...
if False: 
    ...
    # plt.legend()
    fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures', 'mapped_shorelines.png'),dpi=200)
...
geojson_file = os.path.join(inputs['filepath'], inputs['sitename'], 'transects', 'matakana_beach_transects.geojson')
...
fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures', 'time_series_raw.png'),dpi=200)
...
filepath = os.path.join(inputs['filepath'], inputs['sitename'], 'tides', 'topex_tauranga_1984-2024.csv')
...
ax.legend()
fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures', 'tide_data_water_levels.png'),dpi=200)
...
for i,key in enumerate(cross_distance.keys()):
    ...
    ax.text(0.5,0.95, key, bbox=dict(boxstyle="square", ec='k',fc='w'), ha='center',
            va='top', transform=ax.transAxes, fontsize=14)
ax.legend()
fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures','shoreline_change_ts_raw_tidal_correction.png'),dpi=200)


#%% 5. Time-series post-processing

# load mapped shorelines from 1984 (mapped with the previous code)
filename_output = os.path.join(inputs['filepath'], inputs['sitename'], "MATAKANA_All_1984_2024_output.pkl")
...
plt.ylabel('Northings')
plt.grid(linestyle=':', color='0.5')
plt.title('%d shorelines mapped at Matakana Beach from 1999'%len(output['shorelines']))
for i in range(len(output['shorelines'])):
    sl = output['shorelines'][i]
    date = output['dates'][i]
    plt.plot(sl[:,0], sl[:,1], '.', label=date.strftime('%d-%m-%Y'))
for i,key in enumerate(list(transects.keys())):
    ...
    plt.text(transects[key][0,0]-100, transects[key][0,1]+100, key,
                va='center', ha='right', bbox=dict(boxstyle="square", ec='k',fc='w'))
fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures', 'mapped_shorelines_transects.png'),dpi=200)

# load long time-series (1984-2021)
filepath = os.path.join(inputs['filepath'], inputs['sitename'], "transect_time_series_tidally_corrected.csv")
...
fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures', 'otsu_threhsolds.png'),dpi=200)
...
for key in cross_distance.keys():
    ...
    fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures',  'seasonally_averaged_shorelines.png'), dpi=200)
...
for key in cross_distance.keys():
    ...
    fig.savefig(os.path.join(inputs['filepath'], inputs['sitename'], 'output', 'figures',  'monthly_averaged_shorelines.png'), dpi=200)
